...
const Videos = () => {
  ...
  return (
    <Stack className='bg-[#1e1b4b] text-white' direction={['column', 'row']} h={"100vh"}>
      <VStack w={"full"}>
        <video className='p-5' controlsList='nodownload' controls style={{ width: '100%' }} src={videoSrc}></video>

        <VStack  alignItems={'flex-start'} py={1} px={5} w={"full"}>
          <Heading>Sample Videos</Heading>
          <Text >This is a sample video for testing the video player</Text>
        </VStack>
      </VStack>

      <VStack className='max-sm:mt-[-50px] max-sm:h-[300px] leading-normal '   w={['full', 'xl']} alignItems={'stretch'} p={8} spacing={8} overflowY={'auto'}>
        {videosArr.map((item, index) => (
          <Button key={index} onClick={() => setVideoSrc(item)} colorScheme="white" variant={"ghost"}>
            Lecture {index + 1}
          </Button>
        ))}
      </VStack>
